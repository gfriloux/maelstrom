        ...
        if ((!module->post) && (module->state < AZY_SERVER_MODULE_STATE_POST) && module->def && module->def->post)
          {
             ...
             module->rewind_now = EINA_TRUE;
             while (module->rewind_now)
               {
...
#define MODULE_STATE(STATE)                        \
  module->rewind_now = EINA_TRUE;                  \
  module->state = AZY_SERVER_MODULE_STATE_##STATE; \
  continue
        ...
        switch (module->state)
          {
           case AZY_SERVER_MODULE_STATE_INIT:
             ...
             if (module->rewind_now) continue;
             else if (module->run_method)
...
static Eina_Bool
_azy_server_client_not_impl(Azy_Server_Client *client, Azy_Server_Module *module)
{
   Azy_Net_Data data;
   Azy_Net *net;
...
_azy_server_client_get_put_options(Azy_Server_Client *client)
{
   ...
   if ((!module) || (module->state == AZY_SERVER_MODULE_STATE_ERR))
     return _azy_server_client_not_impl(client, module);
   module->executing = EINA_TRUE;
   module->post = EINA_FALSE;
#define MODULE_STATE(STATE)                        \
  module->rewind_now = EINA_TRUE;                  \
        ...
        switch (module->state)
          {
           case AZY_SERVER_MODULE_STATE_INIT:
             ...
             module->recv.data = (void *)EBUF(client->current->buffer);
             module->recv.size = EBUFLEN(client->current->buffer);
             if (net)
               {
                  /* grab the req path before it gets freed */
                  net->http.req.http_path = client->current->http.req.http_path;
                       ...
                       Azy_Net_Transport t)
{
   ...
   if (!client->resume_rpc)
     if ((!azy_content_deserialize_request(content, t, (char *)EBUF(client->current->buffer), EBUFLEN(client->current->buffer))) &&
         (!content->faultcode))
                                ...
                                Ecore_Con_Event_Client_Data *ev)
{
   ...
   if (client->net->overflow)
     {
        client->overflow = client->net->overflow;
   ...
   if (client->overflow && (!client->net->buffer))
     {
        WARN("%s: Calling %s again to try using %" PRIi64 " bytes of overflow data...", client->ip, __PRETTY_FUNCTION__, EBUFLEN(client->overflow));
        _azy_server_client_handler_data(client, type, NULL);
        if (!client->overflow)
          WARN("%s: Overflow has been successfully used!", client->ip);
        else
          WARN("%s: Overflow could not be used, storing %" PRIi64 " bytes for next event.", client->ip, EBUFLEN(client->overflow));
     }
                               ...
                               Ecore_Con_Event_Client_Del *ev)
{
   ...
   if (!ecore_con_server_clients_get(client->server->server))
     {
        if (client->overflow) eina_binbuf_free(client->overflow);
        client->overflow = NULL;
        client->data = NULL;
     }
